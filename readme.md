...
- `loginRedirectTo` - string with the path or url to be used in the redirection done after the login process is finished with success. Example: /dashboard.
- `logoutPath` - string with the path of the route that allows the client to log out. A GET route will be created with this path. Example: '/logout'.
