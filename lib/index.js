...
// (for routes configured with the hapi-auth-session strategy);
// see hapi-auth-cookie docs for more details
internals.validateFunc = function (request, cookieData, callback) {

    // cookieData is an object of the form { uuid: ... }, where uuid is the key/id of the
    // session previously set in the cache;
    ...
    internals.sessionCache.get(uuid, function (err, value, cached, report) {
        ...
        // session data in catbox does not exist or is expired - execute the callback
        ...
        // if we arrived here: there is some session object in the cache whose
        // key is the uuid given by the cookie data; this means the user is authenticated

        // in the request handler we will have access to both the cookie data and the session object
        // - the cookie data object will be available in request.auth.artifacts
        // - the session object will be available in request.auth.credentials
        isValid = true;
...
exports.register = function (server, options, next){
    ...
    internals.sessionCache = server.cache(options.policy);
    const sessionCache = internals.sessionCache;
    ...
    if(options.scheme.validateFunc){
        return next(new Error('The options for "cookie" scheme cannot have the "validateFunc" property'))
    }
        ...
        config: {
            ...
            handler: function (request, reply) {
                ...
                options.validateLoginData(request, function (err, session){
                    ...
                    // 1) set the session in the internal cache
                    const uuid = UUID.v4();
                    ...
                    sessionCache.set(
                        ...
                        function (err){
                            ...
                            // uuid of the session stored in the cache
                            const cookieData = {
                                uuid: uuid
                            };
